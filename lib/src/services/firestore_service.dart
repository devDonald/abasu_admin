...
class FirestoreService {
  ...
  Future<void> addUser(ApplicationUser user) {
    return _db.collection('admins').doc(user.userId).set(user.toMap());
  }
  ...
  Stream<List<Product>> fetchProductsByVendorId(String vendorId) {
    return _db
        .collection('products')
        .where('adminId', isEqualTo: vendorId)
        .snapshots()
  ...
  Future<Admin> fetchAdmin(String adminId){
    return _db
      .collection('admin_profile')
      .doc(adminId)
      .get().then((snapshot) => Admin.fromFirestore(snapshot.data()));
  }

  Future<void> setAdmin(Admin admin){
    var options = SetOptions(merge:true);
  ...
  Future<List<DocumentSnapshot>> getCategories() async =>
      await _db.collection('categories').get().then((snaps) {
        return snaps.docs;
      });

  Future<List<DocumentSnapshot>> getSubCategories(String url) async =>
      await _db.collection(url).get().then((snaps) {
        return snaps.docs;
      });
